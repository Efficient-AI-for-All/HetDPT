"""stage 1 step 1 layer-candidate fine-tuning.
...
from hetdpt.engine_lambda import train_one_epoch, evaluate
...
from hetdpt import models_pruned
from hetdpt import utils
from hetdpt.models_pruned import Block, Block_act_compensation
import re

def extract_numbers(text):
    """_summary_
    Extract the number that appears after `blocks.` in a module path.
    Args:
        text (str): Input string, for example `blocks.3.xxx`.
    
    Returns:
        list: Extracted block indices, for example `[3]`.
    """
...
def get_args_parser():
    parser = argparse.ArgumentParser('HetDPT stage 1 step 1 layer-candidate fine-tuning', add_help=False)
    ...
    parser.set_defaults(repeated_aug=False)  # Set this to True for DeiT-Base.
    ...
    parser.add_argument('--prune_layer', type=str, default='[1,2,3,4]', help='Legacy pruning-layer list kept for compatibility.')
    parser.add_argument('--done_layer', type=str, default='[]', help='Layers that have already been removed in a sequential pruning run.')
    parser.add_argument('--frozen_stages', type=int, default=-1, help='Freeze layers up to this stage; -1 disables freezing.')
    parser.add_argument('--pretrained', action='store_true', default=False, help='Use timm pretrained weights when supported by the selected ViT backbone.')
    parser.add_argument('--prune_layer_act', default='[]', type=str, help='Python-style list of FFN activation-layer indices removed by HetDPT, e.g. "[2,7,8,10,11]".')
    parser.add_argument('--prune_layer_attn', default='[]', type=str, help='Python-style list of attention-layer indices removed by HetDPT, e.g. "[0,3,7,8,11]".')
    return parser
...
def main(args):
    ...
    model = create_model(
        ...
        prune_layer_attn=eval(args.prune_layer_attn),
        prune_layer_act=eval(args.prune_layer_act),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('HetDPT stage 1 step 1 layer-candidate fine-tuning', parents=[get_args_parser()])
